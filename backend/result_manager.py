...
class ResultManager:
    def __init__(self, config_manager):
        ...
        self.is_race_active = False # レース進行中フラグ
    ...
    def reset_realtime_state(self):
        ...
        if self.is_race_active:
            self.race_stop_time = time.monotonic()
            ...
            for p in active_pilots:
                ...
                if p_freq and int(p_freq) in cam_freqs:
                    ...
                    if seat < 4:
                        ...
                        self.realtime_race_state[seat]["is_active"] = True
        except: pass
    ...
    def process_realtime_lap(self, port, lap_data):
        ...
        now_abs = float(lap_data.get("lap_time", time.monotonic()))
        active_ports = {}
        for i in range(1, 5):
        ...
        else: calculated_delta = (now_abs - self.race_start_mono) if self.race_start_mono > 0 else 0.0
        ...
        if server_idx == 1:
            ...
            if state["lap_count"] == -1:
                final_lap_total = (now_abs - self.race_start_mono) if self.race_start_mono > 0 else 0.0
                if is_holeshot_cfg: state["lap_count"], state["is_holeshot_active"] = 0, True
                else: state["lap_count"], state["is_holeshot_active"] = 1, False
            else:
        ...
        ranking = sorted(range(4), key=lambda x: (-self.realtime_race_state[x]["is_active"], -self.realtime_race_state[x]["lap_count"], -self.realtime_race_state[x]["last_sector"], self.realtime_race_state[x]["last_gate_time_abs"]))
        all_pos = {}
    ...
    def calculate_metrics_for_laps(self, laps_list, consec_n, target_n):
        ...
        if laps_list and isinstance(laps_list[0], dict):
            ...
            for lap in laps_sorted:
                ...
                if ln not in seen and ln >= 1: plens.append(lap["lengthSeconds"]); seen.add(ln)
    ...
    def update_all(self):
        try:
            ...
            local_photos = self.load_local_pilot_photos(source_eid); pilots_list = self.fetch_pb("pilots", {"perPage": 500})
            pmap = {p["id"]: {**p, "photoPath": local_photos.get(str(p.get("sourceId", "")).lower().strip().replace("-", "")) or p.get("photoPath")} for p in pilots_list}
            races_all = self.fetch_pb("races", params={"filter": f"event = '{eid}'", "sort": "raceOrder,raceNumber", "perPage": 500}); r_dict = {r["id"]: r for r in races_all}
            ...
            pilot_channels = self.fetch_pb("pilotChannels", params={"filter": f"event = '{eid}'", "perPage": 2000}); channels = {c["id"]: c for c in self.fetch_pb("channels")}
            ...
            if next_race:
                curr_rid = next_race["id"]; pcs = [pc for pc in pilot_channels if pc.get("race") == curr_rid]; cam_freqs = self.config_manager.get("timer", "camera_frequencies") or []
                for pc in pcs:
                    pid = pc.get("pilot"); chid = pc.get("channel"); freq = channels.get(chid, {}).get("frequency")
                    if freq and int(freq) in cam_freqs:
                        seat = cam_freqs.index(int(freq)); official_laps = p_race_map.get(pid, {}).get(curr_rid, [])
                        if official_laps:
                            laps_sorted = sorted(official_laps, key=lambda x: x.get("lapNumber", 0))
                            self.realtime_race_state[seat]["history"] = [l.get("lengthSeconds", 0.0) for l in laps_sorted]
                            ...
                            self.realtime_race_state[seat]["is_active"] = True
            target_ids = {r["id"] for r in races_all if r.get("valid") is not False} if round_id == "all" else ({rid for rid, r in rounds_map.items() if r.get("eventType") == round_id.split(":", 1)[1]} if str(round_id).startswith("type:") else ({round_id} if round_id in rounds_map else {r["id"] for r in races_all if r.get("round") == round_id}))
            disp_ranking = []
            for pid, r_data in p_race_map.items():
                p_target_rids = [rid for rid in r_data if rid in target_ids]
                if not p_target_rids: continue
                ...
                t_val = b.get(sort_by, 9999); d_time = (f"{b['totalLaps']} Laps {b['totalTime']:.3f}s" if sort_by == "totalLaps" else f"{t_val:.3f}s") if t_val < 9000 else ""
            ...
            def build_pilot_list(race_obj, mode):
                ...
                p_list = []; pcs = [pc for pc in pilot_channels if pc.get("race") == race_obj["id"]]
                for pc in pcs:
                    pid = pc.get("pilot"); chid = pc.get("channel")
                    if pid in pmap:
                        ch_obj = channels.get(chid, {}); m_curr = self.calculate_metrics_for_laps(p_race_map.get(pid, {}).get(race_obj["id"], []), consec_n, race_obj.get("targetLaps", consec_n))
                        p_list.append({"pilotId": pid, "pilotName": pmap[pid].get("name", "???"), "photopath": pmap[pid].get("photoPath", ""), "band": ch_obj.get("displayName", "---")[:2], "frequency": ch_obj.get("frequency", 9999), "rank": rank_lookup.get(pid), "displayValue": f"RECORD: {m_curr['bestLap']:.3f}s" if m_curr['bestLap'] < 900 else "RECORD: NO RECORD"})
                p_list.sort(key=lambda x: x.get("frequency", 9999)); return p_list
            ...
            self._cache = {"ranking": disp_ranking, "next_heat": {"name": f"Race{rounds_map.get(next_race['round'], {}).get('roundNumber', 0)}-{next_race['raceNumber']}" if next_race else "---", "pilots": build_pilot_list(next_race, "next")}, "last_heat": {"name": f"Race{rounds_map.get(last_race['round'], {}).get('roundNumber', 0)}-{last_race['raceNumber']}" if last_race else "---", "pilots": build_pilot_list(last_race, "result")}, "meta": {"sort": sort_by, "event_name": event_obj.get("name"), "event_obj": event_obj}}
            return True
        except Exception as e: self.log("System", f"Update Error: {e}"); return False

    def get_round_list(self, event_id_unused=None):
        try:
            events = self.fetch_pb("events", params={"filter": "isCurrent = true"})
            if not events: events = self.fetch_pb("events", params={"sort": "-updated"})
            eid = events[-1]["id"]; rounds = self.fetch_pb("rounds", params={"filter": f"event = '{eid}' && valid = true", "sort": "roundNumber"})
            res = [{"id": "all", "name": "Total (All Sessions)"}]
            event_types = sorted(list({r.get("eventType") for r in rounds if r.get("eventType")}))
            for etype in event_types: res.append({"id": f"type:{etype}", "name": f"All {etype} Sessions"})
            for r in rounds: res.append({"id": r["id"], "name": f"{r.get('eventType', 'Round')} {r.get('roundNumber', '')}"})
            return res
    ...
    def parse_duration_to_seconds(self, duration_str):
        ...
        if ":" in s:
            try: parts = s.split(":"); return float(parts[0]) * 3600 + float(parts[1]) * 60 + float(parts[2])
            except: pass
        try: return float(s)
        except: return 0.0
    ...
    def get_all_sheets_data(self):
        try:
            ...
            p_race_map = {}
            for l in laps_all:
                rid, did, pid = l.get("race"), l.get("detection"), d_to_p.get(l.get("detection"))
                lnum, l_time = l.get("lapNumber", 0), float(l.get("lengthSeconds", 0))
            ...
            for r in races:
                ...
                r_start = r.get("start") or r.get("created", "")
                r_date, r_time = "", ""
                if r_start and not str(r_start).startswith("0001"):
                    try:
                        norm = str(r_start).replace("-", "/").replace("T", " ")
                        parts = norm.split(" ")
                        r_date = parts[0]
                        time_raw = parts[1] if len(parts) > 1 else "00:00:00"
                        r_time_clean = time_raw.split(".")[0].split("+")[0].split("Z")[0]
                        t_parts = r_time_clean.split(":")
                        if len(t_parts) >= 3:
                            r_time = f"{t_parts[0].zfill(2)}:{t_parts[1].zfill(2)}:{t_parts[2].zfill(2)}"
                    except: pass
                ...
                for pid in pilots:
                    ...
                    m2 = self.calculate_metrics_for_laps(p_laps_v, 2, r.get("targetLaps", 3))
                    p_res = res_lookup.get((rid, pid), {})
                    
                    b2 = m2["consecutive"] if m2["consecutive"] < 900 else ""
                    b3 = m3["consecutive"] if m3["consecutive"] < 900 else ""
                    
                    row = [event_name, f"Race{rnd.get('roundNumber', 0)}-{r.get('raceNumber', 0)}", r_date, r_time, pilots[pid].get("name", "???"), p_res.get("position", ""), len(p_laps_v), m3["totalTime"], m3["totalRace"], m3["bestLap"], b2, b3, l_dict.get(0, "")]
                    for i in range(1, 31): row.append(l_dict.get(i, ""))
                    race_result_rows.append(row)
            
            pilot_stats = []; all_individual_laps = []; target_laps_global = int(event_obj.get("laps", 3))
            for pid, p_info in pilots.items():
                p_name = p_info.get("name", "???")
                best1, best2, best3, best_race = {"val": 999.0, "heat": "---"}, {"val": 999.0, "heat": "---"}, {"val": 999.0, "heat": "---"}, {"val": 9999.0, "heat": "---"}
                for rid, laps_dict in p_race_map.get(pid, {}).items():
                    ...
                    r_obj = next((rc for rc in races if rc["id"] == rid), None)
                    if not r_obj: continue
                    rnd = rounds.get(r_obj.get("round"), {}); heat_name = f"Race{rnd.get('roundNumber', 0)}-{r_obj.get('raceNumber', 0)}"
                    for l_t in l_list: all_individual_laps.append({"name": p_name, "val": l_t, "heat": heat_name})
                    t_laps = int(r_obj.get("targetLaps") or target_laps_global)
                    m_lap = min(l_list)
                    if m_lap < best1["val"]: best1 = {"val": m_lap, "heat": heat_name}
                    if len(l_list) >= 2:
                        c2 = min([sum(l_list[i:i+2]) for i in range(len(l_list)-1)])
                        if c2 < best2["val"]: best2 = {"val": c2, "heat": heat_name}
                    if len(l_list) >= 3:
                        c3 = min([sum(l_list[i:i+3]) for i in range(len(l_list)-2)])
                        if c3 < best3["val"]: best3 = {"val": c3, "heat": heat_name}
                    if len(l_list) >= t_laps:
                        r_time = sum(l_list[:t_laps])
                        if r_time < best_race["val"]: best_race = {"val": r_time, "heat": heat_name}
                if best1["val"] < 900: pilot_stats.append({"name": p_name, "b1": best1, "b2": best2, "b3": best3, "brace": best_race})

            return {
                "lapsToDo": target_laps_global, "RaceResult": race_result_rows,
        ...
        except Exception as e: self.log("Sheet", f"Data Error: {e}"); return None
